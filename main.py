...
async def handle(event):
    ...
    # 🤖 Génération IA
    fallback = "ptdr jsp c’que t’dis mdr tu veux quoi au juste ? 😂"
    reply = None
    try:
        ...
        if any(x in reply.lower() for x in ["je suis désolée", "je suis un modèle", "je suis une intelligence"]):
            print("❌ Réponse robotique détectée")
            reply = fallback
    except Exception as e:
        print("⚠️ Erreur OpenAI :", e)
        reply = fallback

    # 🔁 Évite répétition du fallback
    previous_responses = [m["content"] for m in data["messages"] if m["role"] == "assistant"]
    if reply == fallback and fallback in previous_responses:
        print("⛔ Fallback déjà envoyé, on évite le spam.")
        return

    # 💸 Envoie lien PayPal si conditions remplies
